...
describe('Testing the validator', function () {
  let validateCourseInfoSpy;
  let validateUniqueCourseIdSpy;

  beforeEach((done) => {
    validateCourseInfoSpy = sinon.spy(validator, "validateCourseInfo");
    validateUniqueCourseIdSpy = sinon.spy(validator, "validateUniqueCourseId");
    done();
  });

  afterEach((done) => {
    validateCourseInfoSpy.restore();
  ...
  it('1. Validating the course info', function (done) {
    ...
    assert(validateCourseInfoSpy.calledOnce);
    assert(validateUniqueCourseIdSpy.calledOnce);
    done();
  });
...
describe('Validate the course id', function () {
  let validateCourseInfoSpy;
  let validateUniqueCourseIdSpy;
  let validateAverageRatingCourseSpy;

  beforeEach((done) => {
    validateCourseInfoSpy = sinon.spy(validator, "validateCourseInfo");
    validateUniqueCourseIdSpy = sinon.spy(validator, "validateUniqueCourseId");
    validateAverageRatingCourseSpy = sinon.spy(validator, "validateAverageRating");
    done();
  });
  ...
  afterEach((done) => {
    validateCourseInfoSpy.restore();
    validateUniqueCourseIdSpy.restore();
    validateAverageRatingCourseSpy.restore();
    done();
  });
...
describe('Validates the average rating', function () {
  ...
  it('asserts that course rating is present and rating is integer', function(done) {
    ...
    assert(validateAverageRatingCourseSpy.calledOnce);
    expect(response).equal(true);
    done();
  });
